...
import { testTamboApiKey, diagnoseStreamError } from "@/lib/test-tambo-key";
...
export function TamboSetupHelp() {
  ...
  const handleTestKey = async () => {
    ...
    try {
      ...
      console.log("=== 🔬 Tambo API Key Test Result ===");
  ...
  const handleRunDiagnostics = async () => {
    ...
    try {
      console.clear();
      console.log("🔬 Starting Tambo Stream Diagnostics...");
      console.log("Open this console (F12) to view detailed results");
      console.log("=".repeat(60));

      await diagnoseStreamError();

      console.log("=".repeat(60));
      console.log("✅ Diagnostics complete! Review the output above.");
    } catch (error: any) {
          ...
          <div className="ml-8 space-y-2">
            ...
            <div className="bg-neutral-900 border border-white/20 p-3 relative group">
              <code className="text-success text-xs font-mono block">
                NEXT_PUBLIC_TAMBO_TOKEN=your_api_key_here
              </code>
              <button
                onClick={() =>
                  copyToClipboard("NEXT_PUBLIC_TAMBO_TOKEN=your_api_key_here")
                }
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-400 text-xs mb-3">
              ...
              errors, run diagnostics to identify the issue.
            </p>
            <button
              onClick={handleRunDiagnostics}
              disabled={runningDiagnostics}
              className="w-full px-4 py-3 bg-primary/20 border-2 border-primary/50 text-primary font-bold uppercase text-xs tracking-wider hover:bg-primary/30 hover:border-primary transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {runningDiagnostics ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" />
            ...
            <p className="text-gray-500 text-xs mt-2 italic">
              Results will appear in the browser console (press F12)
            </p>
          </div>
        </div>
      </div>
